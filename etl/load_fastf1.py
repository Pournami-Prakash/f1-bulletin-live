...
Usage:
    python load_fastf1.py                        # loads 2025 + 2026
...
MAX_REPLAY_FRAMES    = 140
REPLAY_POINTS_PER_LAP = 18
REPLAY_CHUNK_SIZE    = 200   # rows per commit to avoid Neon timeout

# ── DB helpers ────────────────────────────────────────────────────────────────

def get_conn():
    """Open a Postgres connection with keepalives to prevent Neon timeouts."""
...
def ms(td) -> int | None:
    """Convert pandas Timedelta to milliseconds, None if NaT."""
    if pd.isnull(td):
        return None
...
def load_session(season: int, round_number: int, conn) -> None:
    cur = conn.cursor()
    ...
    # Clear existing child rows (idempotent reruns)
    ...
    # ── Stints (computed from laps) ──────────────────────────────────────────
    print(f"  → Computing stints ...")
    stint_rows = []

    for driver in laps_df["Driver"].dropna().unique():
        drv_laps = (
    ...
    if total_laps > 0:
        ...
        for drv_num in session.drivers:
            ...
            for lap_num, grp in drv_laps.groupby("LapNumber"):
                ...
                sub = tel.iloc[idx].copy()
                sub["LapNumber"] = int(lap_num)
                sub["Driver"]    = drv_code
                pieces.append(sub[["X", "Y", "LapNumber", "Driver"]])

        if pieces:
            df = pd.concat(pieces, ignore_index=True)

            # Normalise coordinates to centre the track
            df["Xn"] = df["X"] - df["X"].mean()
            df["Yn"] = df["Y"] - df["Y"].mean()

            # Compute frame index for animation
            df["SampleIdx"] = df.groupby(["Driver", "LapNumber"]).cumcount()
            max_idx = (
                df.groupby(["Driver", "LapNumber"])["SampleIdx"]
                .transform("max")
                .replace(0, 1)
            )
            df["ProgressInLap"] = df["SampleIdx"] / max_idx
            df["RaceProgress"]  = (df["LapNumber"] - 1 + df["ProgressInLap"]) / total_laps
            df["Frame"] = (df["RaceProgress"] * (MAX_REPLAY_FRAMES - 1)).round().astype(int)
            df = df.sort_values(["Driver", "LapNumber", "SampleIdx"])
            df = df.drop_duplicates(subset=["Driver", "Frame"], keep="last")
            ...
            # Insert in chunks to avoid Neon connection timeout
            ...
            print(f"  → {len(replay_rows)} replay frames saved")
        else:
...
def load_season(season: int, round_filter: int | None = None) -> None:
    ...
    try:
        for _, event in schedule.iterrows():
            ...
            # Reconnect per session to avoid long-lived connection issues
            try:
                conn.closed  # check if still open
            except Exception:
                conn = get_conn()
...
def main():
    ...
    parser.add_argument(
        ...
        default=[2025, 2026],
        help="Seasons to load (default: 2025 2026)",
    )
    parser.add_argument(
        "--round", type=int, default=None,
        help="Load a single round number only",
    )
    ...
    for season in args.seasons:
        ...
        print(f"{'='*50}")
        load_season(season, round_filter=args.round)
