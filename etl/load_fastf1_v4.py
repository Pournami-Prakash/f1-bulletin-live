...
# Distance-sampled replay: 64 frames per lap = ~6m resolution on a 4km circuit
# Drivers will be spread around the circuit proportional to their real gaps
...
def load_replay(session_id: int, session: f1.core.Session, conn) -> None:
    """
    Load telemetry replay resampled by DISTANCE (not time).
    Each lap divided into REPLAY_FRAMES_PER_LAP equal-distance slices.
    Frame 0 = start/finish, frame N/2 = halfway around.
    Drivers are spread around the circuit proportional to their real gaps.

    Uses session.pos_data (dict keyed by driver number) directly —
    lap.get_pos_data() only works on FastF1 Lap objects, not Series rows.
    """
    cur = conn.cursor()
    ...
    # pos_data is a dict: driver_number_str -> Telemetry DataFrame with X, Y, Time cols
    ...
    for drv_num in session.drivers:
        ...
        # pos_data keys are driver numbers as strings
        drv_pos = pos_data.get(str(drv_num))
        if drv_pos is None or drv_pos.empty:
            continue

        # Ensure we have X, Y, Time columns
        if not {"X", "Y", "Time"}.issubset(drv_pos.columns):
            continue

        # Sort position data by time for slicing
        ...
        for _, lap_row in drv_laps.iterrows():
            ...
            if not pd.isnull(lap_time):
                ...
                # Estimate end from next lap start or add a generous buffer
            ...
            # Slice position data for this lap window
            try:
            ...
            # Add cumulative distance along the lap
            # Compute distance as cumulative sum of Euclidean steps
            ...
            # Resample to REPLAY_FRAMES_PER_LAP equal-distance slices
...
def load_session(
    ...
    # ── Practice ──────────────────────────────────────────────────────────────
    if not replay_only and not extras_only and not quali_only:
    ...
    # ── Stop here if fp-only or quali-only ────────────────────────────────────
    if fp_only or quali_only:
    ...
    if not extras_only:
        ...
        # Telemetry replay (distance-sampled)
        if no_replay:
