...
# ML layer — Ridge (small data) + XGBoost (activates at R8+)
...
MC_RUNS       = 500   # Monte Carlo simulations per race

# ── Season weights (keep in sync with feature_engineering.py) ────────────────
SEASON_WEIGHTS = {
    # Only seasons actually in DB — 2021 onwards
    2021: 0.20, 2022: 0.50, 2023: 0.75, 2024: 0.85, 2025: 0.90, 2026: 1.00,
}

# XGBoost activates when we have this many 2026 races of data
XGB_TRIGGER_RACES = 8

# Extra constructor strength discount for teams with major 2026 changes
# Applied ON TOP of the regulation break discount
CONSTRUCTOR_2026_DISCOUNT = {
    'Aston Martin':    0.30,   # Honda PU transition + major restructure
    'Cadillac':        0.50,   # Brand new team, zero baseline
    'Alpine':          0.15,   # New driver lineup + Renault PU questions
    'Williams':        0.10,   # New Mercedes PU relationship
}

# FP session weights for practice pace signal
# FP2 is most representative of race pace (long runs)
FP_SESSION_WEIGHTS = {
    'FP3': 0.50,   # most recent, closest to qualifying
    'FP2': 0.35,   # race simulation runs
...
# Track-specific pit lane delta time (seconds lost in pit lane vs racing)
PIT_LANE_DELTA = {
    'Monaco':         19.0,
    'Singapore':      24.0,
    'Marina Bay':     24.0,
    ...
    'Zandvoort':      19.0,
    'Budapest':       20.0,
    'default':        21.0,   # average pit lane delta
}

# Max position gains cap per race (prevents unrealistic simulation outcomes)
# Based on historical data — very rare to gain more than 12 positions in a race
MAX_POSITION_GAIN = 12

# Pit crew speed by team (seconds, lower = faster)
# Affects pit stop execution time variance
PIT_CREW_SPEED = {
    'Red Bull Racing': 2.0,
    'Mercedes':        2.2,
    'Ferrari':         2.3,
...
# Position-banded traffic effect on tyre life
# Front runners get clean air (better tyre life), backmarkers get dirty air
def traffic_tyre_factor(position: int) -> float:
    if position <= 3:   return 0.92   # P1-3: clean air, 8% better tyre life
    if position <= 5:   return 0.96   # P4-5: mostly clean
    if position <= 10:  return 1.00   # P6-10: neutral
    if position <= 15:  return 1.04   # P11-15: some traffic
    return 1.08                        # P16+: heavy traffic, 8% worse tyre life
...
def load_artifacts() -> dict:
    ...
    for fname, key in [('circuit_elo.json','circuit_elo'),('dnf_survival.json','dnf_survival'),('tyre_deg_gp.json','tyre_deg_gp')]:
...
def _sample_weights(train: pd.DataFrame) -> np.ndarray:
    ...
    rec   = train.apply(lambda r: 2.0 if (r['season'],r['round']) >= (max_r[0], max_r[1]-3) else 1.0, axis=1)
    return (w * rec).values
...
def train_ridge(features_df) -> Pipeline | None:
    ...
    pipe = Pipeline([('imputer', SimpleImputer(strategy='median')),('scaler', StandardScaler()),('model', Ridge(alpha=10.0))])
...
def train_random_forest(features_df) -> Pipeline | None:
    ...
    step(f"  RF trained: {len(train)} rows — top features: {', '.join(f+':'+str(round(v,2)) for f,v in top)}")
    return pipe
...
def predict_ridge(
    ...
    """
    Use trained Ridge model to predict finish positions.
    Returns dict: driver_code -> predicted_position (float)
    """
    if pipe is None:
        return None

    # Build feature matrix from entries
    rows = []
    for e in entries:
        row = {f: e.get(f, np.nan) for f in RIDGE_FEATURES}
        rows.append(row)
    ...
    try:
        ...
        # Clip to valid range and rank
        preds = np.clip(preds, 1, len(entries))
...
# ─────────────────────────────────────────────────────────────
# XGBOOST MODEL (activates at R8+)
# ─────────────────────────────────────────────────────────────
def train_xgboost(features_df: pd.DataFrame, n_2026_races: int) -> object | None:
    """
    Train XGBoost on historical + 2026 data.
    Only called when n_2026_races >= XGB_TRIGGER_RACES.
    Uses sample weights by season.
    """
    ...
    # Season weights as sample weights
...
# Activates after MIN_CALIBRATION_RACES scored races
# Maps raw model output probabilities to empirically-correct ones
# ─────────────────────────────────────────────────────────────
MIN_CALIBRATION_RACES = 10   # need at least 10 scored races to fit calibration

CALIBRATOR_PATH = FEATURES_DIR / 'calibrator.json'

def fit_calibrator() -> None:
    """
    Fit isotonic calibration on past predictions vs actuals.
    Saves calibration lookup to features_output/calibrator.json.
    Run after each race: python predict.py --fit-calibrator
    """
    scored = query("""
        SELECT win_probability, podium_probability,
               actual_position
        FROM predictions
    ...
    if len(scored) < MIN_CALIBRATION_RACES * 5:
        step(f"  Calibration: only {len(scored)} scored rows, need {MIN_CALIBRATION_RACES*5}. Skipping.")
        return
    ...
    # Save as lookup tables (isotonic = piecewise constant)
    calib = {
    ...
    step(f"  Calibration fitted on {len(scored)} rows — saved to calibrator.json")

def load_calibrator() -> dict | None:
    """Load saved calibration lookup if it exists and has enough data."""
    if not CALIBRATOR_PATH.exists():
        return None
    try:
        with open(CALIBRATOR_PATH) as f:
            c = json.load(f)
        if c.get('n_rows', 0) < MIN_CALIBRATION_RACES * 5:
            return None
        return c
    except Exception:
        return None
...
def apply_calibration(predictions: list[dict], calibrator: dict | None) -> list[dict]:
    """
    Apply isotonic calibration to win/podium probabilities.
    Falls back to beta shrinkage if no calibrator fitted yet.
    """
    if calibrator is None:
        # Pre-calibration: light beta shrinkage toward uniform
        # Prevents extreme overconfidence but keeps meaningful differentiation
        # alpha = shrinkage strength: 0.0 = no shrinkage, 1.0 = fully uniform
        # At R2 (data_w=0.2): alpha=0.08 — very light touch
        # At R8 (data_w=0.8): alpha=0.04 — almost none (isotonic calibration takes over)
        n_d      = len(predictions)
        data_w   = predictions[0].get('data_weight_2026', 0.1) if predictions else 0.1
        alpha    = max(0.04, 0.10 - data_w * 0.07)   # 0.04–0.10 range only
        mean_win = 1.0 / n_d
        mean_pod = 3.0 / n_d
    ...
    # Isotonic calibration: interpolate from lookup table
    import numpy as _np
    ...
    # Renormalise win after calibration
    total = sum(p['win_probability'] for p in predictions)
    if total > 0:
        for p in predictions:
            p['win_probability'] = round(p['win_probability'] / total, 4)
...
def project_championship(season: int) -> pd.DataFrame | None:
    """
    Project season championship standings based on:
    - Actual points accumulated so far
    - Expected points for remaining races (from win probability × points value)
    """
    # Get actual standings so far
    actual = query("""
        SELECT r.driver_code, r.team, SUM(r.points) as actual_points,
               COUNT(*) as races_completed
        FROM results r
        JOIN sessions s ON s.id = r.session_id
        WHERE s.season=%s AND s.session_type='R'
    ...
    # Get latest predictions (most recent round)
    latest_pred = query("""
    ...
    races_done      = int(actual['races_completed'].max())
    races_remaining = max(0, RACES_PER_SEASON - races_done)

    # Expected points per remaining race from latest prediction
    proj = actual.merge(latest_pred[['driver_code','points_expected']], on='driver_code', how='left')
    proj['points_expected']  = proj['points_expected'].fillna(2.0)  # ~P12 avg
...
def get_target_race(season: int | None, round_: int | None) -> tuple[int, int, str, str]:
    """Returns (season, round, gp_name, circuit)"""
    ...
    if season and round_:
        ...
        # Not in DB yet (upcoming race) — get from schedule
        import fastf1 as f1
...
def get_entry_list(season: int, round_: int, artifacts: dict) -> list[dict]:
    """
    Build the list of drivers + their pre-race features.
    Uses qualifying data if available, otherwise latest results.
    """
    elo          = artifacts['elo']
    ...
    # Try qualifying data for this race (checks both Q session and results grid)
    quali = query("""
    ...
    if not quali.empty:
        ...
        step(f"  No qualifying data found for {season} R{round_} — using grid from results or estimated order")

    # Get driver/team list: prefer from this race's results, else latest race
    race_results = query("""
    ...
    if not race_results.empty:
        ...
        # Fallback: use qualifying data driver list directly (we already have it)
        if not quali.empty:
            latest_results = quali[['driver_code']].copy()
            # Get team from most recent results
            teams = query("""
            ...
            # Fallback: use most recent CURRENT season race for driver/team list
            # This ensures we get the 2026 grid, not historical drivers
    ...
    circuit_row = query("""
        ...
        WHERE season=%s AND round=%s AND session_type='R'
        LIMIT 1
    """, (season, round_))
    circuit = circuit_row.iloc[0]['circuit'] if not circuit_row.empty else 'Unknown'
    cp = circuits.get(circuit, {})

    # Weather forecast (use avg from same circuit historically if no live data)
    wx = query("""
    ...
    # FP pace signal — weighted blend of FP1/FP2/FP3 median laps
    fp_data = query("""
    ...
    # Compute FP pace gap to fastest FP time (weighted blend)
    fp_pace: dict[str, float] = {}
    if not fp_data.empty:
        for fp_name, fp_w in FP_SESSION_WEIGHTS.items():
            fp_session_data = fp_data[fp_data['fp_session'] == fp_name]
            if fp_session_data.empty:
                continue
            fp_session_data = fp_session_data.dropna(subset=['median_lap_ms'])
            if fp_session_data.empty:
                continue
            ...
            for _, row in fp_session_data.iterrows():
                ...
                # Convert gap to adjustment score: 0ms gap = 0 adj, 1000ms = -0.2 adj
                adj = -min(gap_ms / 5000.0, 0.2) * fp_w
                fp_pace[d] = fp_pace.get(d, 0.0) + adj
        if fp_pace:
            step(f"  FP pace signal loaded for {len(fp_pace)} drivers"
                 f" (FP data from {fp_data['fp_session'].nunique()} sessions)")
    ...
    for i, driver in enumerate(all_drivers):
        ...
        if not q_row.empty:
            ...
            # No qualifying data — estimate grid from Elo ranking
            # Sort drivers by Elo descending = expected qualifying order
            grid_pos       = i + 1   # already sorted by Elo in entry building
            gap_to_pole    = i * 0.22  # ~220ms per position gap estimate
            start_compound = 'SOFT'

        # Circuit Elo blending
        ...
        circuit_aff  = driver_hist[driver_hist.get('circuit', pd.Series()) == circuit]['finish_position'].mean() \
                       if not driver_hist.empty and 'circuit' in driver_hist.columns else rolling_avg

        # Confidence: how much 2026 data we have
        n_2026_races = query("SELECT COUNT(DISTINCT round) as n FROM sessions WHERE season=2026 AND session_type='R'").iloc[0]['n']
        ...
        entries.append({
            ...
            'fp_pace_adj':       fp_pace.get(driver, 0.0),  # FP pace adjustment
    ...
    # If qualifying data available: sort by grid position
    # If estimated: sort by Elo (best predictor of qualifying order)
    if not quali.empty:
        entries.sort(key=lambda x: x['grid_position'])
    else:
        # Sort by Elo descending — best driver gets P1 estimate
        entries.sort(key=lambda x: x['elo_rating'], reverse=True)
        # Re-assign grid positions after Elo sort
...
def bayesian_prior(entries: list[dict]) -> dict[str, float]:
    """
    Compute prior win probability for each driver.
    Circuit-weighted: at low-overtaking tracks (Monaco, Albert Park),
    grid position is amplified; at high-overtaking tracks (Spa, Baku),
    pace/Elo matters more.
    """
    scores = {}
    # Get overtaking index from first entry (circuit-level, same for all)
    overtaking_idx = entries[0].get('overtaking_index', 0.30) if entries else 0.30

    # Circuit-adaptive weights
    # Low overtaking (0.2) → grid_w=0.47, elo_w=0.15
    # High overtaking (0.5) → grid_w=0.35, elo_w=0.20
    grid_w    = 0.35 + (1 - overtaking_idx) * 0.15   # range: 0.35–0.50
    elo_w     = 0.20 - (1 - overtaking_idx) * 0.05   # range: 0.15–0.20
    team_w    = 0.15
    gap_w     = 0.15
    circuit_w = 0.10
    pu_w      = 0.05
    ...
    for e in entries:
        ...
        elo_score     = (e['elo_rating'] - 1300) / 400
        elo_score     = max(0, min(1, elo_score))
        team_score    = 1 - (e['team_strength'] - 1) / max(20 - 1, 1)
        team_score    = max(0, min(1, team_score))
        gap_score     = 1 - min(e['gap_to_pole_sec'] / 5.0, 1)
        circuit_score = 1 - (e['circuit_affinity'] - 1) / max(n - 1, 1)
        circuit_score = max(0, min(1, circuit_score))
        pu_score      = e['pu_strength']

        # Apply 2026 constructor discount for teams with major changes
        team          = e.get('team', '')
        ...
        # FP pace adjustment — if available, shift gap_score
        fp_pace_adj = e.get('fp_pace_adj', 0.0)  # set in get_entry_list if FP data loaded
...
# ─────────────────────────────────────────────────────────────
# MONTE CARLO RACE SIMULATION (v2 — improved)
# ─────────────────────────────────────────────────────────────
def simulate_race(entries: list[dict], race_distance: int = 58, artifacts: dict | None = None) -> dict[str, list[int]]:
    """
    Run MC_RUNS simulated races with:
    - Track-specific pit lane delta times
    - Position-banded traffic effect on tyre life
    - Position-banded overtaking probability (front harder, back easier)
    - Capped total position gains (MAX_POSITION_GAIN)
    - Pit crew speed variance by team
    - Lap 1 incident model weighted by grid position band
    ...
    # Circuit-specific pit delta
    circuit       = entries[0].get('circuit', 'default')
    pit_delta_sec = PIT_LANE_DELTA.get(circuit, PIT_LANE_DELTA['default'])
    # Convert pit delta to position loss estimate (pit delta / avg lap time * positions)
    avg_lap_sec   = 90.0   # rough average
    pit_pos_loss  = pit_delta_sec / avg_lap_sec * n * 0.3  # approx positions lost
    ...
    for run in range(MC_RUNS):
        ...
        sc_active    = False
        sc_laps_left = 0
        sc_count     = 0   # track number of SCs per race

        # Base pace: gap to pole is the main signal, 120ms noise captures race variance
        # Elo adjustment is intentionally small — gap_to_pole already encodes skill
        for e in entries:
            elo_adj  = (1500 - e['elo_rating']) * 0.05
            noise    = np.random.normal(0, 120)
            pace_ms[e['driver_code']] = e['gap_to_pole_sec'] * 1000 + elo_adj + noise

        # ── LAP 1 INCIDENT MODEL ───────────────────────────────
        # P1-3: low risk (clean air, no battles)
        # P4-10: medium risk (turn 1 battles)
        # P11+: high risk (compressed field, T1 chaos)
        ...
        for e in entries:
            ...
            if np.random.random() < lap1_risk[d]:
                # Incident: either retire or lose positions
        ...
        for lap in range(1, race_distance + 1):
            ...
            # SC check — use circuit probability, cap at 3 SCs per race
            if not sc_active and sc_count < 3:
                sc_prob_this_lap = entries[0]['sc_probability'] / race_distance
                if np.random.random() < sc_prob_this_lap:
                    sc_active    = True
            ...
            # Sort drivers by current position for traffic calculations
            pos_sorted = sorted(
            ...
            for i, d in enumerate(pos_sorted):
                ...
                # Undercut threat: if driver immediately behind is faster + fresher tyres
                ...
                should_pit = (
                    ...
                    not sc_active   # don't pit under SC unless desperate
                ) or (
                    # Emergency pit: tyre way past limit
                ...
                if should_pit:
                    ...
                    # Next compound selection (must use ≥2 compounds)
                    used_compounds = set()
                    if pit_count[d] == 1:
                        used_compounds.add(e['start_compound'])
                    if compound == 'SOFT':
                        tyre_type[d] = 'MEDIUM'
                    ...
                    # Pit crew speed affects position loss variance
                ...
                # ── PACE CALCULATION (quadratic tyre deg) ────
                ...
                if sc_active:
                    effective_pace = np.random.normal(0, 50)   # minor gaps under SC

                # ── OVERTAKING (position-banded) ──────────────
                # Front runners: harder to overtake (defensive driving, DRS trains)
                # Backmarkers: easier (less to defend, less scrutiny)
                pos_overtake_factor = (
                    0.4 if current_pos <= 3 else    # very hard at front
                    0.7 if current_pos <= 8 else    # hard in points
                    1.0 if current_pos <= 15 else   # normal
                    1.4                              # easier at back
                )
                base_overtake = e['arw_effectiveness'] * 0.15 * pos_overtake_factor

                for j, other in enumerate(pos_sorted):
                    other_e = next(x for x in entries if x['driver_code'] == other)
                    if other == d or other in retired:
                        continue
                    if positions[d] > positions[other]:  # d is behind other
                        other_pace = (
                            pace_ms[other] +
                            COMPOUND_DEG.get(tyre_type[other], COMPOUND_DEG['MEDIUM'])['deg_ms_per_lap'] * tyre_age[other]
                        )
                        pace_gap = effective_pace - other_pace
                        if pace_gap < -150 and np.random.random() < base_overtake:
                            positions[d]     -= 1
                            positions[other] += 1
        ...
        for d in active:
            ...
            if gained > MAX_POSITION_GAIN:
                # Too unrealistic — reinsert at capped position
                capped_pos = start_pos[d] - MAX_POSITION_GAIN
                final_order.append((d, capped_pos))
            else:
                final_order.append((d, active.index(d) + 1))
...
# ─────────────────────────────────────────────────────────────
# COMPUTE PREDICTIONS FROM SIMULATION RESULTS
# ─────────────────────────────────────────────────────────────
def compute_predictions(
    ...
    data_weight_2026 = min(n_2026 / 10.0, 0.9)

    # ── BLEND WEIGHTS ─────────────────────────────────────────────────────────
    # Win probability: Monte Carlo + Bayesian prior ONLY
    # ML models (Ridge/RF) are position regressors — they produce P7 not 8% win prob
    # Converting regression positions to win% is lossy and flattening — don't do it
    #
    # MC captures race dynamics (SC, overtaking, DNF, strategy)
    # Prior captures pre-race information (grid, Elo, team)
    # R2:  MC=60% Prior=40%
    # R8:  MC=72% Prior=28%
    # R16: MC=82% Prior=18%
    prior_weight = max(0.40 - data_weight_2026 * 0.25, 0.15)
    mc_weight    = 1.0 - prior_weight

    # ML models used ONLY for predicted_position (the median finish estimate)
    # Ridge and RF blend the position estimate alongside MC median
    ridge_weight_pos = 0.20 if ridge_preds else 0.0
    rf_weight_pos    = 0.10 if rf_preds    else 0.0
    mc_weight_pos    = 1.0 - ridge_weight_pos - rf_weight_pos
    ...
    for e in entries:
        ...
        # Predicted position: blend MC median + Ridge + RF
        ...
        blended_median = round(
            mc_weight_pos * median_pos +
            ridge_weight_pos * ridge_pos +
            rf_weight_pos * rf_pos
        )

        # Win/podium probability: MC + Bayesian prior ONLY — clean and sharp
        ...
        # Upset detection: meaningful win% AND significantly above grid expectation
        # Only flag if: grid P5+ AND win% > 8% AND at least 2x grid-implied probability
        grid_expected_win = max(0.001, 1 - (e['grid_position'] - 1) * 0.08)
        upset_ratio   = blended_win / grid_expected_win
        is_upset      = (
            e['grid_position'] >= 5 and
            blended_win >= 0.08 and        # meaningful win chance
            upset_ratio >= 2.0             # at least 2x what grid suggests
        )
        ...
        predictions.append({
            ...
            'data_weight_2026':   round(data_weight_2026, 3),
    ...
    # ── MONOTONIC SMOOTHING  (calibration now applied in main() via apply_calibration) ────────────────────────────────────────────────────
    # Ensure win probabilities decrease monotonically (P1 > P2 > P3 etc.)
    # Small violations from blending are common — smooth them out
    for i in range(1, len(predictions)):
        if predictions[i]['win_probability'] > predictions[i-1]['win_probability']:
            # Average with previous to smooth
    ...
    # Re-normalise win probs after smoothing
    total = sum(p['win_probability'] for p in predictions)
    if total > 0:
        for p in predictions:
            p['win_probability'] = round(p['win_probability'] / total, 4)
...
def write_predictions(predictions: list[dict]) -> None:
    ...
    for p in predictions:
        # Cast all numerics to plain Python types — psycopg2 can't handle np.float64
...
def score_prediction(season: int, round_: int) -> None:
    """Compare predictions to actual results and write accuracy metrics."""
    section(f"Scoring {season} R{round_}")

    # Get latest model version for this round (don't hardcode version)
    version_row = query("""
    ...
    # Update actual positions in predictions table
    conn = get_conn()
    cur  = conn.cursor()
...
def print_predictions(predictions: list[dict], gp_name: str) -> None:
    ...
    print(f"  Model: {MODEL_VERSION}  |  {MC_RUNS} MC sims + Ridge regression  |  Confidence: {predictions[0]['confidence']:.0%}")
    print(f"  {'─'*60}")
...
def main():
    ...
    parser.add_argument("--score",   action="store_true", help="Score a past prediction against actuals")
    parser.add_argument("--dry-run",         action="store_true", help="Print predictions without writing to DB")
    parser.add_argument("--fit-calibrator",  action="store_true", help="Refit calibration curves from scored predictions")
    args = parser.parse_args()

    # Fit calibrator mode
    if args.fit_calibrator:
        step("Fitting calibration curves from scored predictions...")
        fit_calibrator()
        return
    ...
    step(f"Training rows: {meta.get('train_rows', '?')}")

    # Target race
    season, round_, gp_name, circuit = get_target_race(args.season, args.round_)
    step(f"Target: {season} R{round_} — {gp_name} ({circuit})")

    # Check how many 2026 races we have
    n_2026_races = int(query("SELECT COUNT(DISTINCT round) as n FROM sessions WHERE season=2026 AND session_type='R'").iloc[0]['n'])
    step(f"2026 races in DB: {n_2026_races} / {XGB_TRIGGER_RACES} needed for XGBoost")

    # Train ML model — Ridge until R8, then XGBoost
    if XGBOOST_AVAILABLE and n_2026_races >= XGB_TRIGGER_RACES:
        step(f"Training XGBoost (R{n_2026_races} — XGBoost threshold reached)...")
        ml_model   = train_xgboost(artifacts['features'], n_2026_races)
        ml_type    = 'xgboost'
    else:
        step(f"Training Ridge regression (R{n_2026_races} — XGBoost activates at R{XGB_TRIGGER_RACES})...")
        ml_model   = train_ridge(artifacts['features'])
        ml_type    = 'ridge'

    ridge_pipe = ml_model  # keep variable name for compatibility

    # Update model version string to reflect which ML model is active
    ...
    if len(entries) == 0:
        print("  ✗ No drivers found. Check qualifying data is loaded:")
        print("    python load_fastf1_v3.py --seasons 2026 --quali-only")
        return
    if len(entries) < 15:
        print(f"  ⚠️  Only {len(entries)} drivers found — some may be missing")

    # Primary ML model (Ridge or XGBoost)
    if ml_type == 'xgboost':
        ridge_preds = predict_xgboost(ml_model, entries)
        if ridge_preds:
            step(f"  XGBoost predictions computed for {len(ridge_preds)} drivers")
    else:
        ridge_preds = predict_ridge(ridge_pipe, entries)
        if ridge_preds:
            step(f"  Ridge predictions computed for {len(ridge_preds)} drivers")

    # Random Forest — always runs alongside primary ML model
    step("Training Random Forest ensemble...")
    rf_pipe   = train_random_forest(artifacts['features'])
    rf_preds  = predict_ridge(rf_pipe, entries) if rf_pipe else None  # same predict fn works
    if rf_preds:
        step(f"  Random Forest predictions computed for {len(rf_preds)} drivers")

    # Calibration — loads historical accuracy if available (activates after R10)
    ...
    if champ is not None:
        ...
        for _, row in champ.head(5).iterrows():
            print(f"    {row['driver_code']:6s}  actual={int(row['actual_points']):3d}pts  projected={int(row['projected_total']):3d}pts  ({int(row['races_remaining'])} races left)")
    ...
    race_distance = 58  # default, could fetch from API
    step(f"Running {MC_RUNS} Monte Carlo simulations ({race_distance} laps)...")
    sim_results = simulate_race(entries, race_distance, artifacts=artifacts)
    step(f"  Simulations complete")

    # Compute final predictions — blend MC + Ridge + Bayesian
    step("Computing final predictions (MC + Ridge + Bayesian blend)...")
    predictions = compute_predictions(
        entries, sim_results, priors, season, round_, gp_name,
        ridge_preds=ridge_preds, rf_preds=rf_preds
    )

    # Apply calibration (isotonic if fitted, beta shrinkage otherwise)
