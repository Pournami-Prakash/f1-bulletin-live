            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24 }}>
              ...
              {data ? (
                <div style={{ padding: '8px 14px', border: `1px solid ${data.confidence < 0.4 ? 'rgba(245,158,11,.25)' : data.confidence < 0.7 ? 'rgba(56,189,248,.25)' : 'rgba(74,222,128,.25)'}`, borderRadius: 8, background: 'rgba(0,0,0,.3)', textAlign: 'center', minWidth: 100, flexShrink: 0, marginTop: 8 }}>
                  <div style={{ fontSize: 7, color: 'rgba(255,255,255,.25)', fontFamily: mono, letterSpacing: '.12em', marginBottom: 2 }}>CONFIDENCE</div>
                  ...
                  <div style={{ fontSize: 7, color: 'rgba(255,255,255,.2)', fontFamily: mono, letterSpacing: '.1em' }}>{data.confidence < 0.4 ? 'EARLY SEASON' : data.confidence < 0.7 ? 'BUILDING' : 'HIGH'}</div>
                </div>
              ) : (
                <div style={{ minWidth: 100, flexShrink: 0 }} />
              )}
